...
namespace UniqueActualizacionDatos.Data
{
       ...
       public static void Busqueda_Consultora(ref ConsultoraModel objModel )
        {
            ...
            using (SqlConnection connection = new SqlConnection(StaticConnectionString))
            {
                ...
                using (SqlCommand command = new SqlCommand(storedProcName, connection))
                {
                    ...
                    if (objModel.TipoDocumentoId == 1)
                    {
                        command.Parameters.AddWithValue("@vchCodConsultora", objModel.vchDato);
                        command.Parameters.AddWithValue("@vchDNI", System.DBNull.Value);
                    }
                    else
                    {
                        command.Parameters.AddWithValue("@vchDNI", objModel.vchDato);
                        command.Parameters.AddWithValue("@vchCodConsultora", System.DBNull.Value);
                    }
